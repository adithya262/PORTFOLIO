      ...
      {isVisible && (
        ...
        >
          ...
          <a href="#hero" className="flex items-center gap-2 group cursor-pointer">
            <Diamond className="w-4 h-4 text-neon-cyan group-hover:animate-pulse" />
            <span className="text-lg font-sora font-semibold text-slate-50 group-hover:text-neon-cyan transition-colors">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <a 
                key={item.name} 
                href={item.link}
                className="relative text-slate-300 hover:text-neon-cyan font-medium transition-colors text-base"
              >
                {item.name}
